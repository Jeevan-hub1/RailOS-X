...
## Notes
  - [ ] 1.1 Deploy Apache Kafka cluster (3 brokers, replication factor 3, min.insync.replicas=2) on Kubernetes
  - [ ] 1.2 Deploy InfluxDB 3.0 primary instance with hot standby replica and WAL streaming replication
  - [ ] 1.3 Deploy PostgreSQL with Patroni HA (primary + 2 replicas) for audit logs, hazard register, and asset registry
  - [ ] 1.4 Deploy Apache Flink cluster (job manager + task managers) on Kubernetes
  - [ ] 1.5 Deploy Delta Lake storage layer (Apache Parquet on S3-compatible object store) for historical data
  - [ ] 1.6 Deploy PostGIS (PostgreSQL + spatial extensions) for Digital Twin geospatial layer
  - [ ] 1.7 Deploy Keycloak identity provider with TOTP MFA support and 4 RBAC role definitions
  - [ ] 1.8 Deploy Kong API Gateway with JWT validation, rate-limiting (1000 req/min), and versioned routing plugins
  - [ ] 1.9 Deploy HashiCorp Vault with audit device enabled for immutable configuration change logging
  - [ ] 1.10 Deploy MinIO with Object Lock (WORM, COMPLIANCE mode) for forensic evidence storage
  - [ ] 1.11 Deploy MLflow tracking server and artifact store (S3-backed, geo-replicated)
  - [ ] 1.12 Configure Kubernetes cluster with Pod Security Admission (restricted profile) and network policies

- [ ] 2. Time Synchronization Infrastructure
  - [ ] 2.1 Configure GPS-disciplined PTP grandmaster clock at zone compute node
  - [ ] 2.2 Deploy PTP boundary clocks on each station Edge_Node (IEEE 1588)
  - [ ] 2.3 Implement `CLOCK_DRIFT_ALERT` emitter: monitor drift >±100ms, publish to monitoring.alerts Kafka topic
  - [ ] 2.4 Implement `CLOCK_UNRELIABLE` flag injection into sensor event canonical schema when sync is lost
  - [ ] 2.5 Write integration test: simulate clock source failure, verify all subsequent events carry `CLOCK_UNRELIABLE` flag
...
- [ ] 15. Role-Based Access Control and MFA
  - [ ] 15.1 Configure Keycloak: create 4 roles (Operations_Controller, Security_Officer, Engineering_Team, Governance_Officer) with permission scopes per §9.1 design
  - [ ] 15.2 Enable TOTP MFA on all privileged role accounts in Keycloak; configure MFA enforcement policy
  - [ ] 15.3 Configure Kong Gateway JWT plugin: validate RS256 JWT from Keycloak JWKS endpoint; reject unauthenticated requests with HTTP 401
  - [ ] 15.4 Implement role-enforcement middleware on all RailOS service endpoints: deny out-of-scope actions with HTTP 403 + audit log entry (user identity, attempted action, timestamp)
  - [ ] 15.5 Write integration test: verify each role cannot perform actions outside its defined permission scope
...
- [ ] 17. Container Security
  - [ ] 17.1 Apply Kubernetes Pod Security Admission (restricted profile) to all RailOS namespaces: runAsNonRoot, allowPrivilegeEscalation=false, capabilities drop ALL, readOnlyRootFilesystem where feasible
  - [ ] 17.2 Document and record in hazard register any subsystem requiring write access to root filesystem (residual risk acceptance per Req 39 C1)
...
- [ ] 18. Model Governance and CI/CD Pipeline
  - [ ] 18.1 Configure MLflow tracking: all model training runs log parameters, metrics, and artifacts with MAJOR.MINOR.PATCH version tags; link run IDs to requirement IDs in traceability matrix
